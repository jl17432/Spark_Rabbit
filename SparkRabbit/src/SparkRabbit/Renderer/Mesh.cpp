...
namespace SparkRabbit {
	...
	struct LogStream : public Assimp::LogStream
	{
		...
		{
			SPARK_CORE_ERROR("Assimp error: {0}", message);
		}
	};
		...
		: m_FilePath(filename)
	{
		...
		if (!scene || !scene->HasMeshes())
			SPARK_CORE_ERROR("Failed to load mesh file: {0}", filename);
		...
		for (size_t m = 0; m < scene->mNumMeshes; m++)
		{
			...
			SPARK_CORE_ASSERT(mesh->HasPositions(), "Meshes require positions.");
			SPARK_CORE_ASSERT(mesh->HasNormals(), "Meshes require normals.");
			...
			{
				SPARK_CORE_ASSERT(mesh->mFaces[i].mNumIndices == 3, "Must have 3 indices.");
		...
		if (scene->HasMaterials())
		{
			...
			for (uint32_t i = 0; i < scene->mNumMaterials; i++)
			{
				...
				if (hasAlbedoMap)
				{
					...
					{
						SPARK_CORE_ERROR("Could not load texture: {0}", texturePath);
				...
				{
					// TODO: Temp - this should be handled by our own filesystem
					std::filesystem::path path = filename;
					auto parentPath = path.parent_path();
					parentPath /= std::string(aiTexPath.data);
					std::string texturePath = parentPath.string();
	...
	void Mesh::OnUpdate(TickTime ts)
	{
		if (m_IsAnimated)
		{
	...
	{
		SPARK_CORE_ASSERT(pNodeAnim->mNumRotationKeys > 0,"Assertion failed:");
	...
	{
		SPARK_CORE_ASSERT(pNodeAnim->mNumScalingKeys > 0,"Assertion failed:");
	...
	glm::vec3 Mesh::InterpolateTranslation(float animationTime, const aiNodeAnim* nodeAnim)
	{
		...
		SPARK_CORE_ASSERT(NextPositionIndex < nodeAnim->mNumPositionKeys, "Assertion failed:");
		float DeltaTime = (float)(nodeAnim->mPositionKeys[NextPositionIndex].mTime - nodeAnim->mPositionKeys[PositionIndex].mTime);
		float Factor = (animationTime - (float)nodeAnim->mPositionKeys[PositionIndex].mTime) / DeltaTime;
		if (Factor < 0.0f)
			Factor = 0.0f;
		SPARK_CORE_ASSERT(Factor <= 1.0f, "Factor must be below 1.0f");
	...
	glm::quat Mesh::InterpolateRotation(float animationTime, const aiNodeAnim* nodeAnim)
	{
		...
		SPARK_CORE_ASSERT(NextRotationIndex < nodeAnim->mNumRotationKeys,"Assertion failed:");
	...
	glm::vec3 Mesh::InterpolateScale(float animationTime, const aiNodeAnim* nodeAnim)
	{
		...
		SPARK_CORE_ASSERT(nextIndex < nodeAnim->mNumScalingKeys, "Assertion failed");
	...
	void Mesh::ReadNodeHierarchy(float AnimationTime, const aiNode* pNode, const glm::mat4& parentTransform)
	{
		...
		for (uint32_t i = 0; i < pNode->mNumChildren; i++)
			ReadNodeHierarchy(AnimationTime, pNode->mChildren[i], transform);  //Recursive
	}
