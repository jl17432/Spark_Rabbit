...
namespace SparkRabbit {
	...
	void Scene::Update2DBodies(TickTime ts)
	{
	...
	void Scene::Init2DPhysics() {
		...
		for (auto e : view) {
			...
			if (entity.HasComponent<BoxCollider2DComponent>())
			{
				...
				const b2Vec2 centroid = b2Vec2(boxCollider2D.Offset.x, boxCollider2D.Offset.y);
				boxShape.SetAsBox(boxCollider2D.Size.x * transform.Scale.x, boxCollider2D.Size.y * transform.Scale.y, centroid, 0.0f);
	...
	void Scene::Create3DBody(Entity& entity) {
		...
		if (rb3d.Shape == CollisionShape::Box)
		{
			...
			shape = new btBoxShape(btVector3(halfExtent.x * transform.Scale.x, halfExtent.y * transform.Scale.y, halfExtent.z * transform.Scale.z) * 2);
		...
		{
			shape = new btSphereShape(transform.Scale.x * 2);
			if (rb3d.mass > 0.0f) shape->calculateLocalInertia(rb3d.mass, inertia);
		...
		else if (rb3d.Shape == CollisionShape::ConvexHull && entity.HasComponent<MeshComponent>())
		{
			...
			{
				static_cast<btConvexHullShape*>(shape)->addPoint(btVector3(vertex.Position.x * transform.Scale.x, vertex.Position.y * transform.Scale.y, vertex.Position.z * transform.Scale.z) * 2);
			}
