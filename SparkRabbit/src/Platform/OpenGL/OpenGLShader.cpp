...
namespace SparkRabbit {
	...
	std::string OpenGLShader::ReadShaderFromFile(const std::string& filepath) const
	{
		...
		{
			SPARK_CORE_ASSERT(false, "Could not load shader!");
		}
	...
	std::unordered_map<GLenum, std::string> OpenGLShader::PreProcess(const std::string& source)
	{
		...
		while (pos != std::string::npos)
		{
			...
			SPARK_CORE_ASSERT(type == "vertex" || type == "fragment" || type == "pixel" || type == "compute", "Invalid shader type specified");
	...
	int32_t OpenGLShader::GetUniformLocation(const std::string& name) const
	{
		...
		if (result == -1)
			SPARK_CORE_WARN("Could not find uniform '{0}' in shader", name);
	...
	void OpenGLShader::CompileAndUploadShader()
	{
		...
		for (auto& kv : m_ShaderSource)
		{
			...
			if (isCompiled == GL_FALSE)
			{
				...
				SPARK_CORE_ASSERT("Shader compilation failed:\n{0}", &infoLog[0]);

				// We don't need the shader anymore.
				glDeleteShader(shaderRendererID);

				SPARK_CORE_ASSERT(false, "Failed");
			}
		...
		if (isLinked == GL_FALSE)
		{
			...
			SPARK_CORE_ERROR("Shader compilation failed:\n{0}", &infoLog[0]);
	...
	void OpenGLShader::ResolveAndSetUniforms(const std::unique_ptr <OpenGLShaderUniformBufferDeclaration>& decl, Buffer buffer)
	{
		const ShaderUniformList& uniforms = decl->GetUniformDeclarations();
	...
	void OpenGLShader::ResolveAndSetUniform(OpenGLShaderUniformDeclaration* uniform, Buffer buffer)
	{
		...
		SPARK_CORE_ASSERT(uniform->GetLocation() != -1, "Uniform has invalid location!");
		...
		case OpenGLShaderUniformDeclaration::Type::STRUCT:
			...
			break;
		default:
			SPARK_CORE_ASSERT(false, "Unknown uniform type!");
		}
	}
