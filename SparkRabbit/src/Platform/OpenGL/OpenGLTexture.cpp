...
namespace SparkRabbit{
	...
	{
		SPARK_CORE_ASSERT(width && height, "Texture dimensions cannot be 0!");
		Renderer::Submit([this]() 
			{
				...
				GLenum wrap = m_Wrap == TextureWrap::Repeat ? GL_REPEAT : GL_CLAMP_TO_EDGE;
				glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, wrap);
				glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, wrap);
				glTextureParameteri(m_RendererID, GL_TEXTURE_MAX_ANISOTROPY, RenderAPI::GetInfo().MaxAnisotropy);

				glTexImage2D(GL_TEXTURE_2D, 0, TextureFormatToGL(m_Format), m_Width, m_Height, 0, TextureFormatToGL(m_Format), GL_UNSIGNED_BYTE, nullptr);
		...
		:m_FilePath(path)
	{
		...
		{
			SPARK_CORE_INFO("Loading HDR texture {0}, srgb={1}", path, srgb);
			m_ImageData.Data = (uint8_t*)stbi_loadf(path.c_str(), &width, &height, &channels, 0);
			m_IsHDR = true;
			m_Format = TextureFormat::Float16;
		}
		else
		{
			SPARK_CORE_INFO("Loading texture {0}, srgb={1}", path, srgb);
			m_ImageData.Data = stbi_load(path.c_str(), &width, &height, &channels, srgb ? STBI_rgb : STBI_rgb_alpha);
			SPARK_CORE_ASSERT(m_ImageData.Data, "Could not read image!");
		...
		Renderer::Submit([=]()
		{
			...
			else
			{
				...
				GLenum internalFormat = TextureFormatToGL(m_Format);
				GLenum format = srgb ? GL_SRGB8 : (m_IsHDR ? GL_RGB : TextureFormatToGL(m_Format)); // HDR = GL_RGB for now
	...
	void OpenGLTexture2D::Unlock()
	{
		m_Locked = false;
		Renderer::Submit([this]() {
			glTextureSubImage2D(m_RendererID, 0, 0, 0, m_Width, m_Height, TextureFormatToGL(m_Format), GL_UNSIGNED_BYTE, m_ImageData.Data);
			});
	}

	void OpenGLTexture2D::Resize(uint32_t width, uint32_t height)
	{
		SPARK_CORE_ASSERT(m_Locked, "Texture must be locked!");

		m_ImageData.Allocate(width * height * Texture::GetBPP(m_Format));
#ifdef SPARK_DEBUG
		m_ImageData.ZeroInitialize();//for debug
#endif // SPARK_DEBUG
	}
	...
	OpenGLTextureCube::OpenGLTextureCube(TextureFormat format, uint32_t width, uint32_t height)
	{
		...
		Renderer::Submit([=]() {
			...
			glTextureStorage2D(m_RendererID, levels, TextureFormatToGL(m_Format), width, height);
			glTextureParameteri(m_RendererID, GL_TEXTURE_MIN_FILTER, levels > 1 ? GL_LINEAR_MIPMAP_LINEAR : GL_LINEAR);
			glTextureParameteri(m_RendererID, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
		...
		: m_FilePath(path)
	{
		...
		SPARK_CORE_ASSERT(faceWidth == faceHeight, "Non-square faces!");
		...
		Renderer::Submit([=]() {
			...
			glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
			glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_WRAP_R, GL_CLAMP_TO_EDGE);
			glTextureParameterf(m_RendererID, GL_TEXTURE_MAX_ANISOTROPY, RenderAPI::GetInfo().MaxAnisotropy);

			auto format = TextureFormatToGL(m_Format);
